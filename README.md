...
2. 杭州赛暂定的比赛必须开始的时间
   1. 由于杭州赛采用AB场赛制, 在A场比赛时, 下面一场的队伍已经可以在B场设置机器人并准备比赛, 另比赛场地会设置等待区(Queueing Area), 故当A场的比赛结束后, 如果双方队伍准备就绪, B场比赛比赛马上开始, 反之亦然.
   2. 如果有队伍没有准备就绪, 则轻判并且30秒内必须开始比赛.
...
- 阶段: 手动阶段
- Q: 请问碰撞的判罚界定范围?
- A: 在查询游戏手册且参照了以往比赛的经验后, 裁判组暂定判罚规则如下:
    1. 任何抢环或非抢环情况下的恶意冲撞, 不管当时撞车的机器人速率如何, 或被撞者有没有受到损伤, 一律每次重判.
    2. 正常抢环情况下, 恶意冲撞判定规则如下:
        1. 如果撞车的机器人速率高于0.4m/s, 重判
        2. 速率低于0.4m/s的机器人不判罚
        3. 比赛时, 根据主裁的最终判罚为准. 此处规定的速度只是给各位队伍在比赛前练车时提供一个标准.
- 注: 主裁在场上对于本条规则的判罚为最终判罚, 队伍在离场后不得提出异议.
- 注: 裁判会尽力对本条暂定规则做最严格的执行, 以确保遵守规则的队伍的利益. 如果各位导师对本条规则有异议, 欢迎各位导师在比赛前在导师群里 @曹老师/饼干老师 并提出异议.
